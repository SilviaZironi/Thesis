...
real(dp) :: v0(2), a(2)
integer  :: istat
integer(kind=i4b), parameter :: nq=10,np1=16,np2=18,nptot=1+np1+np2,np=np1+1,nx=10,ipf=8,iqf=5,iemax=40,itermax=60
...
real(kind=dp), allocatable :: e(:),eta(:),eig(:)
real(kind=dp), dimension(np) :: h
real(kind=dp), dimension(nptot,nptot,nq) :: t
allocate(e(0:iemax+1), eta(0:iemax+1), eig(0:iemax+1))
...
!parametri del potenziale: default + lettura da file params (file con una riga di 4 numeri)
!v0 = (/ -570.316_dp, 1438.4812_dp /)
!a  = (/   1.55_dp,      3.11_dp    /)



call get_params(v0, a, istat)   ! CLI se ci sono 4 argomenti, altrimenti params.in
if (istat /= 0) stop 2

write(*,'(A,2(1X,ES12.5))') 'DEBUG v0 =', v0(1), v0(2)
write(*,'(A,2(1X,ES12.5))') 'DEBUG a  =', a(1),  a(2)
call flush(6)




! scrivi i parametri usati nel file di output
...
! Solution of Faddeev equations
  ...
  do
     ...
     emev=e(ie)*hbarc/mf
     !write(oup,15) emev
     ! --- standardizza l’output e crea un file sentinella ---
      write(*,'(A,1X,ES12.5,1X,A)') '3-body binding energy =', emev, 'MeV'
      call flush(6)
      open(unit=77, file='energy.out', status='replace', action='write')
      write(77,'(ES24.16)') emev
      close(77)
